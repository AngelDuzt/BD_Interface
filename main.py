...
def supprimer(id):
    conn = psycopg2.connect(host="127.0.0.1",
                            ...
                            port="5432")
    cursor = conn.cursor()
    query = f'''DELETE from {table_name} where {table_key}=%s'''
...
def newindow(root):
    ...
    GLabel_983["fg"] = "#333333"
    GLabel_983["justify"] = "center"
    GLabel_983["text"] = "var11"
    GLabel_983.place(x=30, y=490, width=70, height=25)
    ...
    GButton_816 = tk.Button(root, command=lambda: update(id.get(), var1.get(), var2.get(), var3.get(), var4.get(),
                                                         var5.get(), var6.get(),
                                                         var7.get(), var8.get(), var9.get(), var10.get()))
    GButton_816["bg"] = "#efcf75"
    ft = tkFont.Font(family='Times', size=16)
    GButton_816["font"] = ft
    GButton_816["fg"] = "#009688"
    GButton_816["justify"] = "center"
    GButton_816["text"] = "Изменить"
    ...
    GLabel_568["text"] = "Введите значения"
    GLabel_568.place(x=10, y=10, width=238, height=30)


def update(key, var1, var2, var3, var4, var5, var6, var7, var8, var9, var10):
    conn = psycopg2.connect(host="127.0.0.1",
                            database="postgres",
                            user="postgres",
                            password="postgres",
                            port="5432")
    cursor = conn.cursor()
    if table_name == 'tax_officer':
        query = f'''UPDATE  {table_name} SET tax_officer_id=%s, firstname=%s,secondname=%s, thirdname=%s, post=%s where {table_key}=%s'''
        cursor.execute(query, (key, var1, var2, var3, var4, key))
    conn.commit()
    conn.close()
    display_MyTable()
    print("modifier")
...
def chercher(id):
    conn = psycopg2.connect(host="127.0.0.1",
                            ...
                            port="5432")
    cursor = conn.cursor()
    query = f'''SELECT * FROM {table_name} where {table_key}=%s'''
    cursor.execute(query, (id,))
    ...
    def table(listbox):
        for j in range(total_columns):
            e = tk.Label(listbox, width=int(1011 / (len(header) * 8.425)), height=2, fg='red', font=('Arial', 10, 'bold'), text=row[j], borderwidth=1,
                         relief="groove")
    ...
    listbox.place(x=340, y=30, width=1011
                  , height=32)
    table(listbox)
...
def inserer(id, var1, var2, var3, var4, var5, var6, var7, var8, var9, var10):
    conn = psycopg2.connect(host="127.0.0.1",
                            database="postgres",
                            user="postgres",
                            password="postgres",
    ...
    cursor = conn.cursor()
    if table_name == 'inn':
        query = f'''INSERT INTO {table_name}(inn, region_code, tax_inspection_number, taxpayer_number, check_digits, inn_date, inn_key, tax_officer_id, kpp) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)'''
        cursor.execute(query, (id, var1, var2, var3, var4, var5, var6, var7, var8))
    elif table_name == 'taxpayer':
        query = f'''INSERT INTO {table_name}(inn, pay_status, work_address) VALUES (%s, %s, %s)'''
        cursor.execute(query, (id, var1, var2))
    elif table_name == 'tax_officer':
        query = f'''INSERT INTO {table_name}(tax_officer_id, firstname, secondname, thirdname, post) VALUES (%s, %s, %s, %s, %s)'''
        cursor.execute(query, (id, var1, var2, var3, var4))
    elif table_name == 'data_on_expenses_and_income':
        query = f'''INSERT INTO {table_name}(taxpayer_inn, monthly_income, monthly_expenses, translations) VALUES (%s, %s, %s, %s)'''
        cursor.execute(query, (id, var1, var2, var3))
    elif table_name == 'receipt':
        query = f'''INSERT INTO {table_name}(id_receipt, taxpayer_inn, receipt_date, serial_number, pay_status) VALUES (%s, %s, %s, %s, %s)'''
        cursor.execute(query, (id, var1, var2, var3, var4))
    else:
        query = f'''INSERT INTO {table_name}(snils, firstname, secondname, thirdname, passport_series, passport_number, address, issue_code, taxpayer_inn, birth_date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'''
        cursor.execute(query, (id, var1, var2))
...
def display_MyTable( table = 0):
    conn = psycopg2.connect(host="127.0.0.1",
                            ...
                            port="5432")
    cursor = conn.cursor()
    global table_name, table_key
    if table == 1:
        table_name = 'tax_officer'
        table_key = 'tax_officer_id'
        query = f'''SELECT * FROM {table_name} order by tax_officer_id'''
    elif table == 2:
        table_name = 'data_on_expenses_and_income'
        table_key = 'taxpayer_inn'
        query = f'''SELECT * FROM {table_name} order by taxpayer_inn'''
    elif table == 3:
        table_name = 'taxpayer'
        table_key = 'inn'
        query = f'''SELECT * FROM {table_name} order by inn'''
    elif table == 4:
        table_name = 'receipt'
        table_key = 'taxpayer_inn'
        query = f'''SELECT * FROM {table_name} order by taxpayer_inn'''
    elif table == 5:
        table_name = 'taxpayer_data'
        table_key = 'taxpayer_inn'
        query = f'''SELECT * FROM {table_name} order by taxpayer_inn'''
    else:
        table_name = 'inn'
    ...
    def table(listbox):
        for i in range(0, total_rows):
            for j in range(total_columns):
                e = tk.Label(listbox, width=int(1011 / (len(header) * 8.425)), fg='#3d8f17',
    ...
    global header
    if table_name == 'data_on_expenses_and_income':
        header = ['taxpayer_inn', 'monthly_income', 'monthly_expenses', 'translations']
    elif table_name == 'inn':
        header = ['inn', 'region_code', 'tax_inspection_number', 'taxpayer_number', 'check_digits', 'inn_date', 'inn_key', 'tax_officer_id', 'kpp']
    elif table_name == 'receipt':
        header = ['id_receipt', 'taxpayer_inn', 'receipt_date', 'serial_number', 'pay_status']
    elif table_name == 'tax_officer':
        header = ['tax_officer_id', 'firstname', 'secondname', 'thirdname', 'post']
    elif table_name == 'taxpayer':
        header = ['inn', 'pay_status', 'work_address']
    else:
        header = ['snils', 'firstname', 'secondname', 'thirdname', 'passport_series', 'passport_number', 'address', 'issue_code', 'taxpayer_inn', 'birth_date']
    for k in range(len(header)):
        e = tk.Label(listbox, width=int(1011 / (len(header) * 8.425)), fg='#e27013', font=('Arial', 10, 'bold'), text=header[k], borderwidth=1,
                     relief="groove")
...
def interface(root):
    ...
    GButton_["text"] = "Добавить значение"
    ...
    GLabel_0["text"] = "   Учет налогоплательщиков   "
    ...
    GLabel_172["text"] = "Введите ключ"
    GLabel_172.place(x=360, y=450, width=130, height=30)
    ...
    GLabel_380["text"] = "Введите значение ключа"
    GLabel_380.place(x=360, y=530, width=240, height=30)
    ...
    GButton_725["text"] = "Поиск"
    GButton_725.place(x=990, y=530, width=320, height=30)

    GButton_725 = tk.Button(root, command=lambda: display_MyTable())
    GButton_725["bg"] = "#df58e8"
    ft = tkFont.Font(family='Times', size=14)
    GButton_725["font"] = ft
    GButton_725["fg"] = "#fff"
    GButton_725["justify"] = "center"
    GButton_725["text"] = "INN"
    GButton_725.place(x=340, y=600, width=100, height=30)

    GButton_725 = tk.Button(root, command=lambda: display_MyTable( table = 2))
    GButton_725["bg"] = "#df58e8"
    ft = tkFont.Font(family='Times', size=14)
    GButton_725["font"] = ft
    GButton_725["fg"] = "#fff"
    GButton_725["justify"] = "center"
    GButton_725["text"] = "DEI"
    GButton_725.place(x=490, y=600, width=100, height=30)

    GButton_725 = tk.Button(root, command=lambda: display_MyTable( table = 4))
    GButton_725["bg"] = "#df58e8"
    ft = tkFont.Font(family='Times', size=14)
    GButton_725["font"] = ft
    GButton_725["fg"] = "#fff"
    GButton_725["justify"] = "center"
    GButton_725["text"] = "RECEIPT"
    GButton_725.place(x=640, y=600, width=100, height=30)

    GButton_725 = tk.Button(root, command=lambda: display_MyTable( table = 1 ))
    GButton_725["bg"] = "#df58e8"
    ft = tkFont.Font(family='Times', size=14)
    GButton_725["font"] = ft
    GButton_725["fg"] = "#fff"
    GButton_725["justify"] = "center"
    GButton_725["text"] = "TAX OFFICER"
    GButton_725.place(x=790, y=600, width=150, height=30)

    GButton_725 = tk.Button(root, command=lambda: display_MyTable( table = 3 ))
    GButton_725["bg"] = "#df58e8"
    ft = tkFont.Font(family='Times', size=14)
    GButton_725["font"] = ft
    GButton_725["fg"] = "#fff"
    GButton_725["justify"] = "center"
    GButton_725["text"] = "TAXPAYER"
    GButton_725.place(x=990, y=600, width=150, height=30)

    GButton_725 = tk.Button(root, command=lambda: display_MyTable( table = 5 ))
    GButton_725["bg"] = "#df58e8"
    ft = tkFont.Font(family='Times', size=14)
    GButton_725["font"] = ft
    GButton_725["fg"] = "#fff"
    GButton_725["justify"] = "center"
    GButton_725["text"] = "TAXPAYER DATA"
    GButton_725.place(x=1190, y=600, width=160, height=30)
